...
void myIntArray::resize(int length) {
	...
	//убрал данное условие проверки из своего класса так как у нас присутствует контроль что контейнер не может быть 0-го размера
	//if (_length > 0) {} (было в прототипе задания на платформе)
